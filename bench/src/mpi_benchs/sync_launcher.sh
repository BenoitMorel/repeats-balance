...
launch()
{
  ...
  write "LD_PRELOAD=$preload mpirun -np $threads ./main synchronized_ftt $path_data/$data/$data.phy $path_data/$data/$data.part $states $use_repeats $update_repeats $lookupsize $iterations $randomized $seed"
